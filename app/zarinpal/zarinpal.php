...
class zarinpal
{
    ...
    public function verification($Amount)
    {
        $MerchantID = myappenv::ipg_info['MerchantID'];
        $ZarinGate = false;
        ...
        if (isset($result["Status"]) && $result["Status"] == 100) {
            ...
            // error
            $msg = "پرداخت ناموفق";
            $msg .= "<br />کد خطا : " . $result["Status"];
            $msg .= "<br />تفسیر و علت خطا : " . $result["Message"];
            return [
                'result' => false,
                'msg' => $msg
            ];
    ...
    public function payment(int $Amount, string $Description, string $Mobile, $ref = null, $CallbackURL = null)
    {
        $MerchantID = myappenv::ipg_info['MerchantID'];
        $Email = "";
        if ($CallbackURL == null) {
            $CallbackURL = route('ConfirmPayment', ['pay' => 'ZAR', 'ref' => $ref]);
        }
        $ZarinGate = false;
        $SandBox = $this->SandBox;
