...
class poolkhord
{
    ...
    public function pay_ipg(int $Amount, string $Description, $branch = myappenv::Branch)
    {
        ...
        if ($this->sandbox) {
            ...
            $send_request->ReferenceId = $ClientUniqueId;
            $send_request->Amount = $Amount;
            return $this->receiver($send_request);
        }
    ...
    public function receiver(Request $request)
    {
        ...
        if ($this->sandbox) {
            ...
            $result['bankReferenceId'] = $randomString; //"bankReferenceId" => "GmshtyjwKSslcdiKj7zVUjcR9cwxYRZ9qu+CCYVwmr"
            $result['securePan'] = "622106******1237"; // "securePan" => "622106******1237"
            $result['clientUniqueId'] = $ReferenceId; // "clientUniqueId" => "err3443"
        ...
        if (isset($result['status'])) {
            if ($result['status'] == "Paid") {
                ...
                return redirect()->route('ConfirmPayment', ['pay' => 'pol', 'ref' => $bankReferenceId]);
            }
        }
        Log::info($result);
