...
class persian
{
    ...
    public function PersianNumber($InputNumber)
    {
    ...
    public function EnglishNumber($InputNumber)
    {
    ...
    public function jdate($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
    {
        ...
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                case 'r':
                    $encKey = $this->jdate_words(array('rh' => $date[7], 'mm' => $j_m));
                    $out .= $date[0] . ':' . $date[1] . ':' . $date[6] . ' ' . $date[4] . ' ' . $encKey['rh'] . '، ' . $j_d . ' ' . $encKey['mm'] . ' ' . $j_y;
                    break;
    ...
    public function jstrftime($format, $timestamp = '', $none = '', $time_zone = 'Asia/Tehran', $tr_num = 'fa')
    {
        ...
        for ($i = 0; $i < $sl; $i++) {
            ...
            switch ($sub) {
                ...
                case 'c':
                    $encKey = $this->jdate_words(array('rh' => $date[6], 'mm' => $j_m));
                    $out .= $date[1] . ':' . $date[2] . ':' . $date[5] . ' ' . date('P', $ts) . ' ' . $encKey['rh'] . '، ' . $j_d . ' ' . $encKey['mm'] . ' ' . $j_y;
                    break;
    ...
    public function tr_num($str, $mod = 'en', $mf = '٫')
    {
        ...
        $encKey_a = array('۰', '۱', '۲', '۳', '۴', '۵', '۶', '۷', '۸', '۹', $mf);
        return ($mod == 'fa') ? str_replace($num_a, $encKey_a, $str) : str_replace($encKey_a, $num_a, $str);
    }
    ...
    public function jdate_words($array, $mod = '')
    {
        foreach ($array as $type => $num) {
            ...
            switch ($type) {
                ...
                case 'mm':
                    $encKey = array('فروردین', 'اردیبهشت', 'خرداد', 'تیر', 'مرداد', 'شهریور', 'مهر', 'آبان', 'آذر', 'دی', 'بهمن', 'اسفند');
                    $array[$type] = $encKey[$num - 1];
                    break;
                ...
                case 'rh':
                    $encKey = array('یکشنبه', 'دوشنبه', 'سه شنبه', 'چهارشنبه', 'پنجشنبه', 'جمعه', 'شنبه');
                    $array[$type] = $encKey[$num];
                    break;

                case 'sh':
                    $encKey = array('مار', 'اسب', 'گوسفند', 'میمون', 'مرغ', 'سگ', 'خوک', 'موش', 'گاو', 'پلنگ', 'خرگوش', 'نهنگ');
                    $array[$type] = $encKey[$num % 12];
                    break;

                case 'mb':
                    $encKey = array('حمل', 'ثور', 'جوزا', 'سرطان', 'اسد', 'سنبله', 'میزان', 'عقرب', 'قوس', 'جدی', 'دلو', 'حوت');
                    $array[$type] = $encKey[$num - 1];
                    break;

                case 'ff':
                    $encKey = array('بهار', 'تابستان', 'پاییز', 'زمستان');
                    $array[$type] = $encKey[(int) ($num / 3.1)];
                    break;

                case 'km':
                    $encKey = array('فر', 'ار', 'خر', 'تی‍', 'مر', 'شه‍', 'مه‍', 'آب‍', 'آذ', 'دی', 'به‍', 'اس‍');
                    $array[$type] = $encKey[$num - 1];
                    break;

                case 'kh':
                    $encKey = array('ی', 'د', 'س', 'چ', 'پ', 'ج', 'ش');
                    $array[$type] = $encKey[$num];
                    break;
    ...
    public function MiladiDate($PersianDate)
    {
        $responseY = substr($PersianDate, 0, 4);
        $responseM = $this->get_string_between($PersianDate, '/', '/');
        $temp = substr($PersianDate, strpos($PersianDate, "/") + 1);
        $responseD = substr($temp, strpos($temp, "/") + 1);
        $MainDate = $this->jalali_to_gregorian($responseY, $responseM, $responseD, '-');
        return ($MainDate);
    }
    ...
    public function MyMiladiDate($PersianDate, $AppendH, $AppendM)
    {
        $responseY = substr($PersianDate, 0, 4);
        $responseM = $this->get_string_between($PersianDate, '/', '/');
        $temp = substr($PersianDate, strpos($PersianDate, "/") + 1);
        $responseD = substr($temp, strpos($temp, "/") + 1);
        $MainDate = $this->jalali_to_gregorian($responseY, $responseM, $responseD, '-');
        $MainDate .= " $AppendH:$AppendM:00";
        return ($MainDate);
    }
    public function MyMiladiDateTime($PersianDate, $AppendTime)
    {
        $responseY = substr($PersianDate, 0, 4);
        $responseM = $this->get_string_between($PersianDate, '/', '/');
        $temp = substr($PersianDate, strpos($PersianDate, "/") + 1);
        $responseD = substr($temp, strpos($temp, "/") + 1);
        $MainDate = $this->jalali_to_gregorian($responseY, $responseM, $responseD, '-');
        $MainDate .= " $AppendTime:00";
        return ($MainDate);
    }
    public function MyPersianNow()
    {
        return $this->MyPersianDate(date("Y-m-d H:i:s"), true, 'P');
    }
    public function TodayPersian($Delay = 0)
    {
        return $this->MyPersianDate(date("Y-m-d", strtotime("+$Delay day")), false, 'n');
    }
    public function GetCurentShamsiMonth()
    {
        $StartTime = strtotime(date("Y-m-d"));
        $day = date('d', $StartTime);
        $month = date('m', $StartTime);
        $year = date('Y', $StartTime);
        $ShamsiDate = $this->gregorian_to_jalali($year, $month, $day);
        return ($ShamsiDate[1]);
    }
    public function GetShasmiDate($MiladiDate)
    {
        $MiladiDate = strtotime($MiladiDate);
        $day = date('d', $MiladiDate);
        $month = date('m', $MiladiDate);
        $year = date('Y', $MiladiDate);
        $ShamsiDate = $this->gregorian_to_jalali($year, $month, $day);
        return ($ShamsiDate);

    }
    public function PersianDateText($MiladiDate, $ditail = false, $Output = 'P')
    {
        $ShamsiDate = "";
        $StartTime = strtotime($MiladiDate);
        $H = date('H', $StartTime);
        $M = date('i', $StartTime);
        //	if($H != '0' && $M != '0')
        if ($ditail)
        ...
        $ShamsiDate = "$Day $Month $year ";
        return $ShamsiDate;
    }
    public function PersianMonthToText($MonthNo)
    {
        switch ($MonthNo) {
            case 1:
                $Month = 'فروردین';
    ...
    public function MyPersianArray($MiladiDate)
    {
    ...
    public function MyPersianDate($MiladiDate, $ditail = false, $Output = 'P')
    {
        $ShamsiDate = "";
        $StartTime = strtotime($MiladiDate);
        $H = date('H', $StartTime);
        ...
        $ShamsiDate .= $this->gregorian_to_jalali($year, $month, $day, '/');
        if ($Output == 'P')
            $ShamsiDate = $this->PersianNumber($ShamsiDate);
        return ($ShamsiDate);
    }

    function MyPersianDateSQL($my_sql_date, $Output = 'P')
    {
        $date_time = new DateTime($my_sql_date);
        $ShamsiDate = "";
        $H = $date_time->format('H') . PHP_EOL;
        $M = $date_time->format('i') . PHP_EOL;
        if ($H != '0' && $M != '0')
            $ShamsiDate .= " $M : $H  - ";
    ...
    function PersianDateengNumber($MiladiDate, $Output = 'P')
    {
        $ShamsiDate = "";
        $StartTime = strtotime($MiladiDate);
        $H = date('H', $StartTime);
    ...
    public function int_date($int_date)
    {
        $nowdate = time();
        $dif = $nowdate - $int_date;
        if ($dif > 86400) {
            $date = date("Y-m-d H:i:s", $int_date);
            $date = $this->MyPersianDate($date, true);
            return $date;
        }
        $dif = intval($dif / 60); // to minute
        if ($dif < 60) {
            return $dif . ' دقیقه قبل ';
        }
        $hours = intval($dif / 60); // to minute
        $minutes = $dif - ($hours * 60);
        return " $hours ساعت و $minutes دقیقه قبل";
