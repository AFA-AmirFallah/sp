                        ...
                        <div id="ProductRow_HasProduct" class="card wpa_cards">
                            <div id="ProductRow_{{ $MyOrderTarget['Product']->id }}">
                                ...
                                <div class="col-sm-12">
                                    ...
                                    @endif
                                    <button style="left: 17px;" name="Edit" value="{{ $MyOrderTarget['Product']->id }}"
                                        class="OrderClose" type="submit">
                                        ویرایش سفارش
                                    </button>
                                </div>
                    ...
                    @if ($count > 0)
                        <div style="margin-top: 20px" class="card wpa_cards">
                            <div class="col-sm-12">
                                ...
                                <br>
                                <strong>سود خرید
                                    :</strong>{{ number_format($BenefitTotall / App\Http\Controllers\Credit\currency::GetCurrencyRate()) }}
                                {{ App\Http\Controllers\Credit\currency::GetCurrency() }}
                            </div>
                        </div>
                    @endif
        ...
        <div class="card wpa_cards">
            ...
            <div class="card-body">
                ...
                <div class="card-content card-content-padding pwa_option_box">
                    ...
                    <input onclick="AutoDelever()" type="radio" class="pwa_radio" name="shipping" value="post">
                </div>
                <div class="card-content card-content-padding pwa_option_box">
                    <li>ارسال با پیک</li>
                    <input onclick="PeykDelever()" type="radio" class="pwa_radio" name="shipping" value="non">
                </div>
                <div id="tehrandelever" class="nested card-content card-content-padding pwa_option_box">
                    <li>تحویل حضوری</li>
                    <input onclick="AskAndDelever()" type="radio" class="pwa_radio" name="shipping" value="non">
                </div>
                <br>
    ...
    <div class="nested" style="width: 90%;margin-right: 5%;" id="FinalizeOrder">
        <div class="card wpa_cards">
            ...
            @if ($UserCashCredit >= $TotalPrice)
                @if ($DeleverCondition != null && $DeleverCondition != '#')
                    <button style="color: white;margin-bottom: 10px;width: 90%;margin-right: 4%;" type="submit"
                        name="submit" value="PayFromCredit"  class="btn btn-primary btn-block m-1 mb-3">پرداخت از
                        کیف پول
                    </button>
                @else
                    <button style="color: white;margin-bottom: 10px;width: 90%;margin-right: 4%;" type="submit"
                        name="submit" value="PayFromCredit" disabled class="btn btn-primary btn-block m-1 mb-3">پرداخت از کیف پول
                    </button>
                @endif
            @endif
        </div>
    ...
    <script>
        ...
        function steper($Status) {
            if ($Status == 'FinalizeOrder_Back') {
                $('#FinalizeOrder').addClass('d-none');
                $('#ShowRegistedAddress').removeClass('d-none');
            ...
            if ($Status == 'ShowRegistedAddress_Back') {
                ...
                $('#ShowRegistedAddress').addClass('d-none');
                $('#ListOFProduct').removeClass('d-none');

            }
            if ($Status == 'ListOFProduct') {
                $('#ListOFProduct').addClass('d-none');
                $('#ShowRegistedAddress').removeClass('d-none');
            }
            if ($Status == 'ShowRegistedAddress') {
                ...
                if ($('#AddLocation').is(':checked')) {
                    ...
                    if (window.Province == 1) {
                        $('#tehrandelever').removeClass('d-none');
                    } else {
                        $('#tehrandelever').addClass('d-none');
                    }
                }
            }
            ...
            if ($Status == 'shipping') {
                $('#shipping').addClass('d-none');
                $('#FinalizeOrder').removeClass('d-none');
            }
    ...
    <script>
        ...
        function incQty(ProductId) {
            ...
            if (parseInt($('#ProductCount_' + ProductId).val()) < parseInt($('#ProductMax_' + ProductId).val())) {
                ...
                $('#UpdateBasket').removeClass('d-none');
                $('#contenubtn').addClass('d-none');
        ...
        function AutoDelever() {
            ...
            $.post('<?php echo e(route('ajax')); ?>', {
                ...
                function(data, status) {
                    ...
                    $('#TotalDeleveryPriceFinal').removeClass('d-none');
