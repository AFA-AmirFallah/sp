@foreach ($Consult->get_consulting_Zone($CatID) as $Zone)
    @if ($loop->first)
        <section class="container mt-3 p-4 max-900 ">
            <h5 id="headingtitle" class="IRANSansWeb_Medium bt-color text-center mb-4">گفتگو و مشاوره با بهترین
                {{ $Zone->Name }}
            </h5>
    @endif
    @php
    ...
    @endphp
    <a onclick="l2laoding({{ $CatID }},{{ $Zone->L2ID }})">
        <div class="row box-transform">
            <div class="col-lg-12 value-body text-center mb-3 p-3 brbtm cursor-pointer">
                <div class="row">
                    <div class="col-md-3 pt-3 pr-3 cursor-pointer ">

                        <span>گفتگو و مشاوره با <b class="IRANSansWeb_Medium">{{ $Zone->Name }}</b></span>
                    </div>
