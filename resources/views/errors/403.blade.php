@if (App\myappenv::SiteTheme == 'Theme1')
    @include('Layouts.Theme1.error403')
@else
    <!DOCTYPE html>
    ...
    <body>
        <div class="not-found-wrap text-center">
            ...
            <p class="text-36 subheading mb-3">{{ __('Access deny') }}</p>
            @if ($exception->getMessage() != null)
                <p class="mb-5  text-muted text-18">
                    {!! $exception->getMessage() !!}</p>
            @else
                <p class="mb-5  text-muted text-18">
                    <p class="mb-5  text-muted text-18">{{ __('Sorry you have not Access permission to brows this URL !!') }}
            @endif
            <div class="row">
